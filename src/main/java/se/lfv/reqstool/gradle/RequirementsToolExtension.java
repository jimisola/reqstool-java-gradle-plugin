...
/**
 * Extension for configuring the Reqstool Gradle Plugin.
 * Provides zero-configuration defaults matching the Maven plugin behavior.
 */
public class RequirementsToolExtension {
    ...
    public RequirementsToolExtension(Project project) {
        ...
        this.requirementsAnnotationsFile.convention(
            project.getLayout().getBuildDirectory()
                .file("generated/sources/annotationProcessor/java/main/resources/annotations.yml")
        );
        this.svcsAnnotationsFile.convention(
            project.getLayout().getBuildDirectory()
                .file("generated/sources/annotationProcessor/java/test/resources/annotations.yml")
        );
        this.outputDirectory.convention(
            project.getObjects().fileProperty().fileProvider(
                project.getLayout().getBuildDirectory().map(d -> d.dir("reqstool").getAsFile())
            )
        );
        this.datasetPath.convention(
            project.getLayout().getProjectDirectory().file("reqstool")
        );
